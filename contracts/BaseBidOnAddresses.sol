...
// FIXME: Two-level collateral transfer (see `salary` branch) is buggy:
// If somebody asks to transfer the collateral from a higher level oracle, it may be too late as the lower level may be already redeemed.
// This can be solved by remebering the amount of each redeemed collateral for each user.
